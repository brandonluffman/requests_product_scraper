...
def get_source(url):
    ...
    try:
        ...
        print(f"Request Recevied -- User Agent: {headers} -- Proxy: {proxy}")
        return response
...
def parse_results(response):
    ...
    review_short_text = '.g1lvWe div'
    review_text = '.g1lvWe div:nth-of-type(2)'
    review_date = ".OP1Nkd span"
    review_rating = ".OP1Nkd div"
    review_source = ".sPPcBf span:nth-of-type(2)"
    ...
    for result in results:
        ...
        review_content = result.find(review_text, first=True)

        if review_content == None:
            item = {
                'review_text' : (result.find(review_short_text, first=True).text),
                'review_rating' : result.find(review_rating, first=True).attrs['aria-label'],
                'review_date' : result.find(review_date, first=True).text,
                'review_source' : result.find(review_source, first=True).text,
            }
            output.append(item)
        else:
            item = {
                'review_text' : result.find(review_text, first=True).text,
                'review_rating' : result.find(review_rating, first=True).attrs['aria-label'],
                'review_date' : result.find(review_date, first=True).text,
                'review_source' : result.find(review_source, first=True).text,
            }
...
urls = ['https://www.google.com/shopping/product/11807794315385323242/reviews?q=bose+quiet+comfort+45&prds=eto:25396480943154138_0;12813554779527938774_0;13511600142849774066_0,pid:3490716478665632251,rsk:PC_17370419192671336164,cs:1&sa=X&ved=0ahUKEwjpgezDkdT9AhXsMVkFHTbwBbwQqSQIcw', 'https://www.google.com/shopping/product/14556986932651510680/reviews?hl=en&q=sony+wh-1000xm5&prds=eto:17832498348390085438_0;16734284565725029642_0;7614542217126070872_0,pid:1061495521773557507,rsk:PC_11648784580466805463&sa=X&ved=0ahUKEwiqyu-nwtT9AhWTKlkFHU6kCHYQqSQIcw']
...
for url in urls:
    urls_count +=1 
    url_reviews[f'URL #{urls_count} ----------- {url}'] = google_search(url)
    print(url_reviews)
    print(f'\n\n-----------------------\n\n')
